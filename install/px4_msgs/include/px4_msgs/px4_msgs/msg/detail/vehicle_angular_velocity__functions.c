  ...
  px4_msgs__msg__VehicleAngularVelocity__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4_msgs__msg__VehicleAngularVelocity * data =
      (px4_msgs__msg__VehicleAngularVelocity *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__VehicleAngularVelocity__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__VehicleAngularVelocity__fini(&output->data[i]);
        }
        return false;
      }
    }
